...
class SolarWindIonSimulation:
    def __init__(self, params=None):
        ...
        self.params = {
            ...
            'dt': 0.02,  # Time step (normalized to Ωp^-1)
    ...
    def calculate_fields(self, z, t):
        ...
        for j in range(self.params['N']):
            ...
            Ex += -E_amplitude * np.sin(phase)  # -90 degree phase shift for left-hand polarization
    ...
    def boris_push(self, z, v, t, dt):
        ...
        new_z = z + 0.5 * dt * (v[2] + new_v[2])  # Only z-component needed
        
        return new_z, new_v

    def run_simulation(self, particle_index=0):
        """Run simulation for a single particle using Boris algorithm"""
        # Time points
        dt = self.params['dt']
        ...
        solution = np.zeros((n_steps, 4))  # [z, vx, vy, vz]
        
        # Set initial conditions
        z = self.z[particle_index]
        v = np.array([
            self.vx[particle_index],
            self.vy[particle_index],
            self.vz[particle_index]
        ])
        
        # Store initial state
        solution[0] = [z, v[0], v[1], v[2]]
        
        # Time integration using Boris algorithm
        for i in range(1, n_steps):
            z, v = self.boris_push(z, v, t[i-1], dt)
            solution[i] = [z, v[0], v[1], v[2]]
    ...
    def calculate_energy(self, v):
        ...
        --------
        energy : float
            Kinetic energy (normalized)
        """
        return 0.5 * np.sum(v**2)
    
    def analyze_energy_conservation(self, solution):
        """
        ...
        -----------
        solution : array-like
            Simulation results for a single particle
        ...
        """
        n_steps = solution.shape[0]
        energy = np.zeros(n_steps)
        
        for i in range(n_steps):
            v = solution[i, 1:4]  # Extract velocity components
            energy[i] = self.calculate_energy(v)
    ...
    def plot_energy_conservation(self, t, solution, num_particles=5):
        ...
        t : array-like
            Time points
        solution : array-like
            Simulation results for multiple particles
        num_particles : int
...
if __name__ == "__main__":
    ...
    #a sim.plot_energy_conservation(t, all_solutions, num_particles=5)